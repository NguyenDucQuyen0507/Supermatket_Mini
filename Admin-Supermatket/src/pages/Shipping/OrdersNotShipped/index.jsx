...
const index = () => {
  ...
  const [ordersShiped, setOrdersShiped] = React.useState([]);
  ...
  React.useEffect(() => {
    ...
    axiosClient
      ...
      .then((response) => {
        ...
        setOrdersShiped(orderConfirmed);
      })
      .catch((err) => {
        logger.error(err);
      });
        ...
        return (
          <Space>
            {users.roles.some((role) => {
              ...
              <Popconfirm
                title="Bạn có muốn nhận đơn hàng này không?"
                onConfirm={() => {
                  //Cancel order
                  const id = record._id;
                  axiosClient
                ...
                cancelText="Không"
              >
                <Button className="border border-green-500">
                  Nhận đơn hàng
                </Button>
              </Popconfirm>
            )}
  ...
  return (
    <>
      <h1 className="p-2 mb-5 text-xl">Đơn hàng chưa vận chuyển 📦</h1>
      <Modal
        centered
      ...
      {users.roles.some((role) => {
        ...
        <div>
          <h1 className="p-2 mb-5 text-xl">Đơn hàng vận chuyển đã nhận 🎁</h1>
          <Table
            rowKey={"_id"}
            dataSource={ordersShiped}
            columns={columnOrderShipped}
          />
        </div>
      ) : (
        ""
